...
@~/.claude/fuckit/references/git-integration.md
</required_reading>
...
2. Use Task tool with subagent_type="fuckit-executor" and model="{executor_model}":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="fuckit-executor" and model="{executor_model}":
...
See `~/.claude/fuckit/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ~/.claude/fuckit/references/checkpoints.md for complete checkpoint guidance.
</step>
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `~/.claude/fuckit/templates/user-setup.md`.
...
Use ~/.claude/fuckit/templates/summary.md for structure.
...
**Also available:**
- `/fuckit:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
**Also available:**
- `/fuckit:verify-work {Z}` — manual acceptance testing before continuing
- `/fuckit:discuss-phase {Z+1}` — gather context first
...
**Also available:**
- `/fuckit:verify-work` — manual acceptance testing before completing milestone
- `/fuckit:add-phase <description>` — add another phase before completing
