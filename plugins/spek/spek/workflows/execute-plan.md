...
@~/.claude/spek/references/git-integration.md
</required_reading>
...
2. Use Task tool with subagent_type="spek:executor" and model="{executor_model}":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="spek:executor" and model="{executor_model}":
...
See `~/.claude/spek/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ~/.claude/spek/references/checkpoints.md for complete checkpoint guidance.
</step>
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `~/.claude/spek/templates/user-setup.md`.
...
Use ~/.claude/spek/templates/summary.md for structure.
...
**Also available:**
- `/spek:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
- `/spek:discuss-phase {Z+1}` — gather context first
...
**Also available:**
- `/spek:verify-work` — manual acceptance testing before completing milestone
- `/spek:add-phase <description>` — add another phase before completing
