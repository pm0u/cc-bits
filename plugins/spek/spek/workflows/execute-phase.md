...
if [ -n "$PLAN_MODEL" ] && echo "$PLAN_MODEL" | grep -qE "^(opus|sonnet|haiku)$"; then
  ...
  EXECUTOR_MODEL=$(lookup_from_table "spek:executor" "$MODEL_PROFILE")
fi
```
...
Run /spek:repair-state to fix, or continue anyway?
```
...
Options:
- Reset failures: /spek:repair-state (clears failure counts)
...
**No dependency analysis needed.** Wave numbers are pre-computed during `/spek:plan-phase`.
...
2. **Read files and spawn agents:**
   ...
   **If `PARALLELIZATION=false`:** Spawn agents one at a time, waiting for each to complete before starting the next. This ensures no concurrent file modifications or build operations.
   ...
   <execution_context>
   @~/.claude/spek/workflows/execute-plan.md
   @~/.claude/spek/templates/summary.md
   @~/.claude/spek/references/checkpoints.md
   @~/.claude/spek/references/tdd.md
   </execution_context>
...
1. **Spawn agent for checkpoint plan:**
   ```
   Task(prompt="{subagent-task-prompt}", subagent_type="spek:executor", model="{executor_model}")
   ```
...
| `gaps_found` | Present gap summary, offer `/spek:plan-phase {phase} --gaps` |
...
- `/spek:verify-work {X}` — manual testing before planning
```

User runs `/spek:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `/spek:execute-phase {X} --gaps-only`
...
1. Run `/spek:execute-phase {phase}` again
