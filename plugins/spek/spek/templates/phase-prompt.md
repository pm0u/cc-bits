...
<!-- For checkpoint task examples and patterns, see @~/.claude/spek/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/spek:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/spek/templates/user-setup.md` for full schema and examples
...
See `~/.claude/spek/workflows/verify-phase.md` for verification logic.
