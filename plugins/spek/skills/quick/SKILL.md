---
name: spek:quick
description: Execute a quick task with SPEK guarantees (atomic commits, state tracking) but skip optional agents
argument-hint: ""
allowed-tools:
  - Read
  - Write
...
<objective>
Execute small, ad-hoc tasks with SPEK guarantees (atomic commits, STATE.md tracking) while skipping optional agents (research, plan-checker, verifier).

Quick mode is the same system with a shorter path:
- Spawns spek:planner (quick mode) + spek:executor(s)
- Skips spek:phase-researcher, spek:plan-checker, spek:verifier
...
<execution_context>
Orchestration is inline - no separate workflow file. Quick mode is deliberately simpler than full SPEK.
</execution_context>
...
Check that an active SPEK project exists:

```bash
if [ ! -f .planning/ROADMAP.md ]; then
  echo "Quick mode requires an active project with ROADMAP.md."
  echo "Run /spek:new-project first."
  exit 1
fi
```
...
Spawn spek:planner with quick mode context:
...
Spawn spek:executor with plan reference:
...
Ready for next task: /spek:quick
```
