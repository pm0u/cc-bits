...
The spek-tools CLI handles all file parsing, directory renaming, and updates.
</execution_context>
...
- Example: `/spek:remove-phase 17` → phase = 17
- Example: `/spek:remove-phase 16.1` → phase = 16.1

If no argument provided:

```
ERROR: Phase number required
Usage: /spek:remove-phase <phase-number>
Example: /spek:remove-phase 17
```
...
```bash
RESULT=$(node ~/.claude/plugins/marketplaces/spek/bin/spek-tools.js phase remove "$PHASE_NUM" 2>&1)
...
INIT=$(node ~/.claude/plugins/marketplaces/spek/bin/spek-tools.js config validate)
...
Would you like to:
- `/spek:progress` — see updated roadmap status
