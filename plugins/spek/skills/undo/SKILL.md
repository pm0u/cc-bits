...
**Next steps:**
- `/spek:execute-phase {PHASE_NUM}` — re-run the plan
- `/spek:plan-phase {PHASE_NUM}` — replan if approach was wrong
- `/spek:progress` — check current status
```
</step>
...
3. Try reset instead (rewrites history): /spek:undo {PLAN_ID} --hard-reset
```
