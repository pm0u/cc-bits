...
<objective>
Create all phases necessary to close gaps identified by `/spek:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/spek:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns spek:planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/spek:audit-milestone` first.
```
...
**Also available:**
- `/spek:execute-phase {N}` — if plans already exist
...
`/spek:audit-milestone` — re-audit to verify gaps closed
`/spek:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/spek:plan-phase` next
</success_criteria>
