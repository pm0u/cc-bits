...
Output: {phase}-UAT.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for /spek:execute-phase
</objective>
...
8. If issues found:
   ...
   - Spawn spek:planner in --gaps mode to create fix plans
   - Spawn spek:plan-checker to verify fix plans
   - Iterate planner ↔ checker until plans pass (max 3)
   - Present ready status with `/clear` then `/spek:execute-phase`
</process>
...
- /spek:execute-phase {Z+1} — skip to execution (if already planned)
...
/spek:execute-phase {Z} --gaps-only
...
- /spek:plan-phase {Z} --gaps — regenerate fix plans
...
**Options:**
- /spek:plan-phase {Z} --gaps — retry fix planning with guidance
- /spek:discuss-phase {Z} — gather more context before replanning
...
- [ ] If issues: spek:planner creates fix plans from diagnosed gaps
- [ ] If issues: spek:plan-checker verifies fix plans (max 3 iterations)
- [ ] Ready for `/spek:execute-phase` when complete
</success_criteria>
