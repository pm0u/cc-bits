# FUCKIT - Agentic Development for Claude Code
...
FUCKIT orchestrates complex development through a hierarchy of agents that spawn fresh context windows, plan work, execute with atomic commits, and verify results:
...
- Optionally maps existing codebase (`/fuckit:map-codebase`)
...
`/fuckit:go` automatically advances to next phase. Just keep running it.
...
`fuckit-tools.js` handles mechanical operations (state parsing, config validation, phase indexing) so skills and agents can focus on orchestration. Single-read architecture eliminates redundant file access.

## Commands

### Primary Workflow

- `/fuckit:new-project` - Initialize project with deep context gathering
- `/fuckit:new-milestone` - Generate roadmap from requirements
- `/fuckit:go` - Smart router (just keep running this)

### Phase Management

- `/fuckit:plan-phase N` - Plan phase N with research and verification
- `/fuckit:execute-phase N` - Execute phase N with wave-based parallelization
- `/fuckit:discuss-phase N` - Interactive discussion before planning
- `/fuckit:research-phase N` - Standalone research before planning
- `/fuckit:list-phase-assumptions N` - Surface assumptions before planning
- `/fuckit:add-phase` - Add phase to end of roadmap
- `/fuckit:insert-phase` - Insert urgent work as decimal phase (e.g., 72.1)
- `/fuckit:remove-phase` - Remove a future phase and renumber

### Progress and Session

- `/fuckit:status` - Visual state diagram
- `/fuckit:progress` - Show progress and suggest next action
- `/fuckit:pause-work` - Create context handoff when pausing mid-phase
- `/fuckit:resume-work` - Resume from previous session with full context

### Verification and Testing

- `/fuckit:verify-work` - Conversational UAT for built features
- `/fuckit:audit-milestone` - Audit milestone completion before archiving
- `/fuckit:plan-milestone-gaps` - Create phases to close audit gaps

### Utilities

- `/fuckit:quick` - Quick task with atomic commits, skip optional agents
- `/fuckit:debug` - Systematic debugging with persistent state
- `/fuckit:map-codebase` - Analyze codebase with parallel mapper agents
- `/fuckit:repair-state` - Reconcile state from git history
- `/fuckit:undo` - Rollback a completed plan
- `/fuckit:add-todo` / `/fuckit:check-todos` - Todo management
- `/fuckit:complete-milestone` - Archive and close milestone

### Configuration

- `/fuckit:settings` - Configure workflow toggles and model profile
- `/fuckit:set-profile` - Switch model profile (quality/balanced/budget)
- `/fuckit:update` - Update to latest version
- `/fuckit:help` - Show available commands
...
FUCKIT uses 11 specialized agents, each spawned with fresh context:
...
**1.1.x**
- CLI delegation via fuckit-tools.js
