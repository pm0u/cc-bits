...
The fuckit-tools CLI handles all file parsing, directory creation, and updates.
</execution_context>
...
- Example: `/fuckit:add-phase Add authentication` → description = "Add authentication"
- Example: `/fuckit:add-phase Fix critical performance issues` → description = "Fix critical performance issues"

If no arguments provided:

```
ERROR: Phase description required
Usage: /fuckit:add-phase <description>
Example: /fuckit:add-phase Add authentication system
```
...
Call the fuckit-tools CLI to perform the operation:

```bash
RESULT=$(node ~/.claude/plugins/marketplaces/fuckit/bin/fuckit-tools.js phase add "$DESCRIPTION" 2>&1)
...
**Also available:**
- `/fuckit:add-phase <description>` — add another phase
- Review roadmap
...
- Don't use decimal numbering (that's /fuckit:insert-phase)
- Don't create plans yet (that's /fuckit:plan-phase)
