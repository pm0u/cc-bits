...
<objective>
Create all phases necessary to close gaps identified by `/fuckit:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/fuckit:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns fuckit:planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/fuckit:audit-milestone` first.
```
...
**Also available:**
- `/fuckit:execute-phase {N}` — if plans already exist
...
`/fuckit:audit-milestone` — re-audit to verify gaps closed
`/fuckit:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/fuckit:plan-phase` next
</success_criteria>
