...
Example: `/fuckit:insert-phase 72 Fix critical auth bug`
→ after = 72
...
if [ $# -lt 2 ]; then
  ...
  echo "Usage: /fuckit:insert-phase <after> <description>"
  echo "Example: /fuckit:insert-phase 72 Fix critical auth bug"
  exit 1
fi
...
```bash
RESULT=$(node ~/.claude/plugins/marketplaces/fuckit/bin/fuckit-tools.js phase insert "$AFTER_PHASE" "$DESCRIPTION" 2>&1)
...
<anti_patterns>
- Don't use this for planned work at end of milestone (use /fuckit:add-phase)
