...
<execution_context>
Orchestration is inline - no separate workflow file. Quick mode is deliberately simpler than full FUCKIT.
</execution_context>
...
Check that an active FUCKIT project exists:

```bash
if [ ! -f .planning/ROADMAP.md ]; then
  echo "Quick mode requires an active project with ROADMAP.md."
  echo "Run /fuckit:new-project first."
  exit 1
fi
```
...
Ready for next task: /fuckit:quick
```
