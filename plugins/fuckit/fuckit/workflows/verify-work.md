<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /fuckit:plan-phase --gaps.
...
<template>
@~/.claude/fuckit/templates/UAT.md
</template>
...
Provide a phase number to start testing (e.g., /fuckit:verify-work 4)
```
...
- `/fuckit:plan-phase {next}` — Plan next phase
- `/fuckit:execute-phase {next}` — Execute next phase
```
</step>
...
- Follow @~/.claude/fuckit/workflows/diagnose-issues.md
...
<downstream_consumer>
Output consumed by /fuckit:execute-phase
...
3. Abandon (exit, user runs /fuckit:plan-phase manually)
...
`/clear` then `/fuckit:execute-phase {phase} --gaps-only`
