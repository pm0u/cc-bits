...
The `huckit-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/huckit/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting huckit context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `huckit-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
Huckit for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/huckit/` (see updated installation docs).
...
**Strengthened using-huckit skill for explicit skill requests**
...
The underlying skills (`huckit:brainstorming`, `huckit:executing-plans`, `huckit:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-huckit` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-huckit`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `huckit:brainstorming`)
...
**Skill priority in using-huckit**
...
- **Rationalizations table** in using-huckit - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-huckit is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > huckit
...
- Optimized huckit bootstrap to eliminate redundant skill execution. The `using-huckit` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-huckit` manually despite already having the content from session start.
...
- Added unified `huckit-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `huckit:skill-name` for huckit skills, `skill-name` for personal
- Personal skills override huckit skills when names match
...
- Updated terminology: "Huckit skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/huckit-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/huckit-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core huckit functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-huckit skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-huckit/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-huckit skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-huckit/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `huckit:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `huckit:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `huckit:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `huckit:code-reviewer`
...
- All internal skill references now use `huckit:` namespace prefix
- Updated format: `huckit:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/huckit:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# Huckit v2.0.0 Release Notes

## Overview

Huckit v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([pm0u/cc-bits-skills](https://github.com/pm0u/cc-bits-skills)). This transforms huckit from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [pm0u/cc-bits-skills](https://github.com/pm0u/cc-bits-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/huckit/skills/`
...
1. Your old `~/.config/huckit/.git` will be backed up to `~/.config/huckit/.git.bak`
2. Old skills will be backed up to `~/.config/huckit/skills.bak`
3. Fresh clone of pm0u/cc-bits-skills will be created at `~/.config/huckit/skills/`

### Removed Features

- **Personal huckit overlay system** - Replaced with git branch workflow
- **setup-personal-huckit hook** - Replaced by initialize-skills.sh
...
- Clones pm0u/cc-bits-skills on first run
...
- `HUCKIT_SKILLS_ROOT` set to `~/.config/huckit/skills`
...
- Removed obsolete setup-personal-huckit call from session-start
...
- Prominent link to huckit-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in pm0u/cc-bits-skills
- `scripts/` directory - Now in pm0u/cc-bits-skills/skills/using-skills/
- `hooks/setup-personal-huckit.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/huckit/skills
- `commands/brainstorm.md` - Updated paths to HUCKIT_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to HUCKIT_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to HUCKIT_SKILLS_ROOT
...
- PR #2: Personal huckit overlay system (later replaced)
...
```bash
# In Claude Code
/plugin marketplace add pm0u/cc-bits-marketplace
/plugin install huckit@huckit-marketplace
```
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/huckit/skills ~/huckit-skills-backup
   ```

2. **Update the plugin:**
   ```bash
   /plugin update huckit
   ```
...
- Skills repository is now at https://github.com/pm0u/cc-bits-skills
...
**Full Changelog:** https://github.com/pm0u/cc-bits/compare/dd013f6...main
**Skills Repository:** https://github.com/pm0u/cc-bits-skills
**Issues:** https://github.com/pm0u/cc-bits/issues
