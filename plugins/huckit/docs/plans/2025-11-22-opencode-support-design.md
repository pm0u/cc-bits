...
Add full huckit support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port huckit to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/huckit-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/huckit.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/huckit/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/huckit-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "huckit:brainstorming")')
  }),
...
1. **Inject using-huckit content**
   - Full content of the using-huckit skill
...
```javascript
// .opencode/plugin/huckit.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const HuckitPlugin = async ({ client, directory, $ }) => {
  const huckitDir = path.join(process.env.HOME, '.config/opencode/huckit');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingHuckit = await readSkill('using-huckit');
...
├── .codex/
│   ├── huckit-codex        # UPDATED: Use skills-core
│   ├── huckit-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── huckit.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/huckit-codex`
...
2. Update `.codex/huckit-codex` to use shared core
...
1. Create `.opencode/plugin/huckit.js`
