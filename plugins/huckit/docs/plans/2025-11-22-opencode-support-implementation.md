...
**Goal:** Add full huckit support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/huckit-codex` (lines 40-74)
...
- Reference: `.codex/huckit-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'huckit' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/huckit-codex` (lines 212-280)
 ...
 * (personal skills override huckit skills).
 *
 * @param {string} skillName - Name like "huckit:brainstorming" or "my-skill"
 * @param {string} huckitDir - Path to huckit skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, huckitDir, personalDir) {
    // Strip huckit: prefix if present
    const forceHuckit = skillName.startsWith('huckit:');
    const actualSkillName = forceHuckit ? skillName.replace(/^huckit:/, '') : skillName;

    // Try personal skills first (unless explicitly huckit:)
    if (!forceHuckit && personalDir) {
    ...
    // Try huckit skills
    if (huckitDir) {
        const huckitPath = path.join(huckitDir, actualSkillName);
        const huckitSkillFile = path.join(huckitPath, 'SKILL.md');
        if (fs.existsSync(huckitSkillFile)) {
            return {
                skillFile: huckitSkillFile,
                sourceType: 'huckit',
                skillPath: actualSkillName
            };
        }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/huckit-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/huckit-codex` (add import at top)
...
Run: `node -c .codex/huckit-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/huckit-codex
...
**Files:**
- Modify: `.codex/huckit-codex` (lines 40-74)
...
Run: `.codex/huckit-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/huckit-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/huckit-codex` (lines 16-38, approximately)
...
Run: `.codex/huckit-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/huckit.js`
...
const huckitSkillsDir = path.join(homeDir, '.config/opencode/huckit/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const HuckitPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/huckit.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/huckit.js
...
**Files:**
- Modify: `.opencode/plugin/huckit.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "huckit:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > huckit)
...
Run: `node -c .opencode/plugin/huckit.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the huckit and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const huckitSkills = skillsCore.findSkillsInDir(
            huckitSkillsDir,
            'huckit',
            3
          );
          ...
          const allSkills = [...personalSkills, ...huckitSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install huckit skills to ~/.config/opencode/huckit/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'huckit:';
...
```javascript
    'session.started': async () => {
      // Read using-huckit skill content
      const usingHuckitPath = skillsCore.resolveSkillPath(
        'using-huckit',
        huckitSkillsDir,
        personalSkillsDir
      );

      let usingHuckitContent = '';
      if (usingHuckitPath) {
        const fullContent = fs.readFileSync(usingHuckitPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingHuckitContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Huckit skills: \`huckit:skill-name\` (from ~/.config/opencode/huckit/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/huckit')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/huckit && git pull` to update huckit.' :
        '';
...
**Below is the full content of your 'huckit:using-huckit' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```markdown
# Installing Huckit for OpenCode
...
### 1. Install Huckit Skills

```bash
# Clone huckit skills to OpenCode config directory
mkdir -p ~/.config/opencode/huckit
git clone https://github.com/pm0u/cc-bits.git ~/.config/opencode/huckit
```

### 2. Install the Plugin

The plugin is included in the huckit repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/huckit/.opencode/plugin/huckit.js`
...
ln -s ~/.config/opencode/huckit/.opencode/plugin/huckit.js .opencode/plugin/huckit.js
```
...
```
use use_skill tool with skill_name: "huckit:brainstorming"
```
...
Personal skills override huckit skills with the same name.
...
1. Check plugin file exists: `ls ~/.config/opencode/huckit/.opencode/plugin/huckit.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/huckit/skills`
...
- Documentation: https://github.com/pm0u/cc-bits
```
...
Huckit works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/huckit-codex use-skill huckit:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/huckit-codex bootstrap | head -30`
...
ls -l .opencode/plugin/huckit.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/huckit.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/huckit-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/huckit-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/huckit.js` created with tools and hooks
