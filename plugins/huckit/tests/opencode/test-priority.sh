...
# Verifies that skills are resolved with correct priority: project > personal > huckit
...
# 1. Create in huckit location (lowest priority)
mkdir -p "$HOME/.config/opencode/huckit/skills/priority-test"
cat > "$HOME/.config/opencode/huckit/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Huckit version of priority test skill
---
# Priority Test Skill (Huckit Version)

This is the HUCKIT version of the priority test skill.

PRIORITY_MARKER_HUCKIT_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/huckit/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Huckit version exists"
else
    echo "  [FAIL] Huckit version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides huckit
echo ""
echo "Test 2: Testing personal > huckit priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides huckit)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_HUCKIT_VERSION"; then
    echo "  [FAIL] Huckit version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|huckit" | head -10
fi

# Test 3: Test that project overrides both personal and huckit
echo ""
echo "Test 3: Testing project > personal > huckit priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Huckit version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit huckit: prefix bypasses priority
echo ""
echo "Test 4: Testing huckit: prefix forces huckit version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load huckit:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_HUCKIT_VERSION"; then
    echo "  [PASS] huckit: prefix correctly forces huckit version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] huckit: prefix did not force huckit version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
