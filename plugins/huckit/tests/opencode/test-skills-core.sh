...
# Create skills in personal and huckit locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/huckit-skills/shared-skill"
mkdir -p "$TEST_HOME/huckit-skills/unique-skill"
...
cat > "$TEST_HOME/huckit-skills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
description: Huckit version
---
# Huckit Shared
EOF

cat > "$TEST_HOME/huckit-skills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
description: Only in huckit
---
# Unique
EOF
...
const huckitDir = '$TEST_HOME/huckit-skills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', huckitDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: huckit: prefix should force huckit
const forced = resolveSkillPath('huckit:shared-skill', huckitDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to huckit
const unique = resolveSkillPath('unique-skill', huckitDir, personalDir);
console.log('UNIQUE:', JSON.stringify(unique));

// Test 4: Non-existent skill
const notfound = resolveSkillPath('not-a-skill', huckitDir, personalDir);
console.log('NOTFOUND:', JSON.stringify(notfound));
" 2>&1)

if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow huckit skills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"huckit"'; then
    echo "  [PASS] huckit: prefix forces huckit resolution"
else
    echo "  [FAIL] huckit: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"huckit"'; then
    echo "  [PASS] Unique huckit skills are found"
else
    echo "  [FAIL] Unique huckit skills not found"
    exit 1
fi
