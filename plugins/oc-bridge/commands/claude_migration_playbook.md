...
This is a step-by-step tutorial for integrating OpenCode into an existing Claude Code setup without duplicating your commands, agents, or skills. Use this as a practical checklist.

## 0) Goal and mental model
- Claude Code assets live in `.claude/` and `~/.claude/`
- OpenCode discovers skills, agents, and commands under `.opencode/` and `~/.config/opencode/`
- We wrap or symlink Claude assets so OpenCode can reuse them
...
2. Add a global skills symlink:
   ...
   ```
3. Add global agents and commands folders:
   ```bash
...
- OpenCode supports Claude-compatible skill discovery via:
  ...
  - `~/.claude/skills/<name>/SKILL.md`
- By symlinking `.opencode/skill` to `.claude/skills` and the global equivalent, OpenCode discovers the same skills.
...
- Deny `write` and `edit` for review agents
...
2. Check skills:
   - Use the `skill` tool list and verify the skills appear.
3. Check commands:
...
- Add frontmatter plus an `@` reference
...
- Skill not found: verify `SKILL.md` frontmatter has `name` and `description`.
- Command not listed: ensure the wrapper file has frontmatter.
...
That is enough for OpenCode and Claude Code to share the same skills, agents, and command content while each tool keeps its own expected format.
