...
/**
 * SHIPIT Tools — CLI utility for SHIPIT workflow operations
 *
 ...
 *
 * Usage: node shipit-tools.js <command> [args]
 *
 * Commands:
...
function resolveModel(profile, agentType) {
  const MODEL_PROFILES = {
    'shipit:executor':           { quality: 'opus', balanced: 'sonnet', budget: 'sonnet' },
    'shipit:planner':            { quality: 'opus', balanced: 'opus',   budget: 'sonnet' },
    'shipit:phase-researcher':   { quality: 'opus', balanced: 'sonnet', budget: 'haiku' },
    'shipit:project-researcher': { quality: 'opus', balanced: 'sonnet', budget: 'haiku' },
    'shipit:verifier':           { quality: 'sonnet', balanced: 'sonnet', budget: 'haiku' },
    'shipit:plan-checker':       { quality: 'sonnet', balanced: 'sonnet', budget: 'haiku' },
    'shipit:debugger':           { quality: 'opus', balanced: 'sonnet', budget: 'sonnet' },
  };
...
function cmdInitExecutePhase(cwd, phase, includes) {
  ...
  const result = {
    ...
    models: {
      executor: resolveModel(config.model_profile, 'shipit:executor'),
      verifier: resolveModel(config.model_profile, 'shipit:verifier'),
    },
...
function cmdInitPlanPhase(cwd, phase, includes) {
  ...
  const result = {
    ...
    // Models
    models: {
      researcher: resolveModel(config.model_profile, 'shipit:phase-researcher'),
      planner: resolveModel(config.model_profile, 'shipit:planner'),
      checker: resolveModel(config.model_profile, 'shipit:plan-checker'),
    },
...
function cmdPhaseAdd(cwd, description) {
  if (!description) {
    ...
    console.error('Usage: shipit-tools phase add <description>');
    process.exit(1);
  }
  ...
  const phaseSection = [
    ...
    '',
    'Plans:',
    `- [ ] TBD (run /shipit:plan-phase ${nextPhase} to break down)`,
    '',
    '**Details:**',
...
function cmdPhaseRemove(cwd, phaseNum) {
  if (!phaseNum) {
    ...
    console.error('Usage: shipit-tools phase remove <phase-number>');
    process.exit(1);
  }
...
function cmdPhaseInsert(cwd, afterPhase, description) {
  if (!afterPhase || !description) {
    ...
    console.error('Usage: shipit-tools phase insert <after> <description>');
    process.exit(1);
  }
  ...
  const phaseSection = [
    ...
    '',
    'Plans:',
    `- [ ] TBD (run /shipit:plan-phase ${decimalPhase} to break down)`,
    '',
    '**Details:**',
    '[To be added during planning]',
    ''
