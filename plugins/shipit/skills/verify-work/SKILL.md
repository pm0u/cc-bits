...
Output: {phase}-UAT.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for /shipit:execute-phase
</objective>
...
8. If issues found:
   ...
   - Spawn shipit:planner in --gaps mode to create fix plans
   - Spawn shipit:plan-checker to verify fix plans
   - Iterate planner ↔ checker until plans pass (max 3)
   - Present ready status with `/clear` then `/shipit:execute-phase`
</process>
...
- /shipit:execute-phase {Z+1} — skip to execution (if already planned)
...
/shipit:execute-phase {Z} --gaps-only
...
- /shipit:plan-phase {Z} --gaps — regenerate fix plans
...
**Options:**
- /shipit:plan-phase {Z} --gaps — retry fix planning with guidance
- /shipit:discuss-phase {Z} — gather more context before replanning
...
- [ ] If issues: shipit:planner creates fix plans from diagnosed gaps
- [ ] If issues: shipit:plan-checker verifies fix plans (max 3 iterations)
- [ ] Ready for `/shipit:execute-phase` when complete
</success_criteria>
