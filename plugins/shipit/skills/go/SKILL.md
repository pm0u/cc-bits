...
Unlike `/shipit:progress` which shows status and suggests commands, `/shipit:go` determines and executes the next action directly.
</objective>
...
Then invoke: `Skill(skill: "shipit:new-project")`
...
Invoke: `Skill(skill: "shipit:new-project")`

**Missing ROADMAP.md but PROJECT.md exists:**

Between milestones. Invoke: `Skill(skill: "shipit:new-milestone")`
</step>
...
1. gaps_found > 0?
   ...
   → Action: Skill(skill: "shipit:plan-phase", args: "{phase} --gaps")

2. summaries < plans?
   → YES: Unexecuted plans exist
   → Action: Skill(skill: "shipit:execute-phase", args: "{phase}")

3. plans = 0?
   → YES: Phase not yet planned
   → Check context:
     - CONTEXT.md exists: Skill(skill: "shipit:plan-phase", args: "{phase}")
     - No CONTEXT.md: Skill(skill: "shipit:discuss-phase", args: "{phase}")
...
4. summaries = plans AND plans > 0?
   ...
   → Check if more phases in milestone:
     ...
     - No more phases: Skill(skill: "shipit:complete-milestone")
```
...
Proceeding with: `/shipit:{command} {args}`
...
- "Show status" - Run /shipit:progress instead
- "Cancel" - Do nothing

**On "Continue":** Invoke the determined Skill

**On "Show status":** Invoke `Skill(skill: "shipit:progress")`
...
- Continue debug session (`/shipit:debug`)
...
```
If handoff exists, invoke `Skill(skill: "shipit:resume-work")` instead.
