...
**Next steps:**
- `/shipit:execute-phase {PHASE_NUM}` — re-run the plan
- `/shipit:plan-phase {PHASE_NUM}` — replan if approach was wrong
- `/shipit:progress` — check current status
```
</step>
...
3. Try reset instead (rewrites history): /shipit:undo {PLAN_ID} --hard-reset
```
