---
name: shipit:plan-milestone-gaps
...
<objective>
Create all phases necessary to close gaps identified by `/shipit:audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/shipit:add-phase` per gap.
</objective>

<execution_context>
<!-- Spawns shipit:planner agent which has all planning expertise baked in -->
</execution_context>
...
```
No audit gaps found. Run `/shipit:audit-milestone` first.
```
...
**Also available:**
- `/shipit:execute-phase {N}` — if plans already exist
...
`/shipit:audit-milestone` — re-audit to verify gaps closed
`/shipit:complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/shipit:plan-phase` next
</success_criteria>
