...
@~/.claude/plugins/marketplaces/shipit/shipit/workflows/execute-phase.md
</execution_context>
...
/shipit:execute-phase {X}  (remove --dry-run)
...
0. **Resolve Model Profile**
   ...
   **Model lookup table:**
   ...
   |-------|---------|----------|--------|
   | shipit:executor | opus | sonnet | sonnet |
...
4. **Execute waves**
   ...
   - Spawn `shipit:executor` for each plan in wave (parallel Task calls)
...
7. **Verify phase goal**
   ...
   **Otherwise:**
   - Spawn `shipit:verifier` subagent with phase directory and goal
   - Verifier checks must_haves against actual codebase (not SUMMARY claims)
   - Creates VERIFICATION.md with detailed report
   - Route by status:
     - `passed` → continue to step 8
     - `human_needed` → present items, get approval or feedback
     - `gaps_found` → present gaps, offer `/shipit:plan-phase {X} --gaps`
...
/shipit:discuss-phase {Z+1} — gather context and clarify approach
...
**Also available:**
- /shipit:plan-phase {Z+1} — skip discussion, plan directly
- /shipit:verify-work {Z} — manual acceptance testing before continuing
...
**Also available:**
- /shipit:verify-work — manual acceptance testing
- /shipit:complete-milestone — skip audit, archive directly
...
- /shipit:verify-work {Z} — manual testing before planning

───────────────────────────────────────────────────────────────

---

After user runs /shipit:plan-phase {Z} --gaps:
1. Planner reads VERIFICATION.md gaps
2. Creates plans 04, 05, etc. to close gaps
3. User runs /shipit:execute-phase {Z} again
...
```
Task(prompt="Execute plan at {plan_01_path}\n\nPlan:\n{plan_01_content}\n\nProject state:\n{state_content}\n\nLessons from prior phases:\n{lessons_content}", subagent_type="shipit:executor", model="{executor_model}")
...
See `@~/.claude/plugins/marketplaces/shipit/shipit/workflows/execute-phase.md` step `checkpoint_handling` for complete details.
</checkpoint_handling>
