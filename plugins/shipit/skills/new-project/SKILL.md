...
**After this command:** Run `/shipit:plan-phase 1` to start execution.
...
1. **Abort if project exists:**
   ```bash
   [ -f .planning/PROJECT.md ] && echo "ERROR: Project already initialized. Use /shipit:progress" && exit 1
   ```
...
- options:
  - "Map codebase first" — Run /shipit:map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**
```
Run `/shipit:map-codebase` first, then return to `/shipit:new-project`
```
Exit command.
...
**Note:** Run `/shipit:settings` anytime to update these preferences.
...
Spawn 4 parallel shipit:project-researcher agents with rich context:
...
/shipit:discuss-phase 1 — gather context and clarify approach
...
**Also available:**
- /shipit:plan-phase 1 — skip discussion, plan directly
...
- [ ] shipit:roadmapper spawned with context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] ROADMAP.md created with phases, requirement mappings, success criteria
- [ ] STATE.md initialized
- [ ] REQUIREMENTS.md traceability updated
- [ ] User knows next step is `/shipit:discuss-phase 1`
