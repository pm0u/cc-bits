...
<!-- For checkpoint task examples and patterns, see @~/.claude/shipit/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/shipit:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/shipit/templates/user-setup.md` for full schema and examples
...
See `~/.claude/shipit/workflows/verify-phase.md` for verification logic.
