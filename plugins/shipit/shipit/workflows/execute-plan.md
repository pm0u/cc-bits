...
@~/.claude/shipit/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node ~/.claude/plugins/marketplaces/shipit/bin/shipit-tools.js init execute-phase "${PHASE}" --include=state,config)
```
...
2. Use Task tool with subagent_type="shipit:executor" and model="{executor_model}":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="shipit:executor" and model="{executor_model}":
...
See `~/.claude/shipit/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ~/.claude/shipit/references/checkpoints.md for complete checkpoint guidance.
</step>
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `~/.claude/shipit/templates/user-setup.md`.
...
Use ~/.claude/shipit/templates/summary.md for structure.
...
**Also available:**
- `/shipit:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
- `/shipit:discuss-phase {Z+1}` — gather context first
...
**Also available:**
- `/shipit:verify-work` — manual acceptance testing before completing milestone
- `/shipit:add-phase <description>` — add another phase before completing
